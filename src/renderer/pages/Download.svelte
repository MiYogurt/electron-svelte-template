...
<div class="wrap">
    ...
    <label>爬取网址</label>
    <input bind:value=url /> {/if}
    <div class="oprator">
        {#if !$start}
        <button on:click="download()">下载</button> {:else}
        <button on:click="stop()">中断</button> {/if}
    </div>

    {#if status.type }
    <div class="type">
        {status.type == 'crawl' && status.step == 'chapter' ? '爬取章节': ''} {status.type == 'crawl' && status.step == 'text'? '爬取内容': ''} {status.type == 'audio'? '转换音频': ''}
    </div>
    {/if} {#if status.title}
    <div class="title">
        {status.title}
        ...
        methods: {
            ...
            blueCircle() {
                ...
                this.context.arc(
                    this.centerX,
                    this.centerY,
                    100, -Math.PI / 2, -Math.PI / 2 + status.percent * this.rad,
                    false
                );
                this.context.stroke();
                this.context.restore();
